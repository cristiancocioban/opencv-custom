"""
Move rows for specified Video_IDs from a validation CSV into a training CSV.

Mirror of 16_move_videos_train_to_val.py — same safety net, same flags,
opposite direction. Use it to undo an earlier move, to shrink the validation
set when it's grown too large, or to recover a clip whose absence from
training is hurting the model (e.g., a crossover-dense video that was
moved to val and is now causing under-counts at inference).

Both CSVs must share the same column schema; matching rows are removed from
validation and appended to training. Originals are backed up to `*.bak` and
overwritten in place.

Run:
    # Defaults point at the *_z.csv pair used by 60_build_gru_model.py.
    python 17_move_videos_val_to_train.py --videos crossover_07_05_01.mp4 crossover_02_05_7.mp4

    # Custom CSV paths:
    python 17_move_videos_val_to_train.py \\
    ...
    python 17_move_videos_val_to_train.py --videos crossover_07_05_01.mp4 --dry-run
"""
...
def move_videos(train_csv: Path, val_csv: Path, video_ids: list[str], dry_run: bool) -> None:
    ...
    not_in_val       = [v for v in requested if v not in val_video_set]
    already_in_train = [v for v in requested if v in train_video_set]

    if not_in_val:
        log.warning("Video_ID(s) not found in validation set, skipping: %s", not_in_val)
    if already_in_train:
        log.warning("Video_ID(s) ALREADY present in training set — moving them "
                    "from validation will create duplicate rows in train: %s", already_in_train)

    to_move = [v for v in requested if v in val_video_set]
    if not to_move:
        raise SystemExit("No requested Video_ID(s) found in the validation CSV. Nothing to do.")

    move_mask = val_df["Video_ID"].isin(to_move)
    moving_rows  = val_df[move_mask]
    keeping_rows = val_df[~move_mask]
    ...
    new_train_df = pd.concat([train_df, moving_rows], ignore_index=True)
    new_val_df   = keeping_rows.reset_index(drop=True)

    log.info("Result: val %d -> %d rows; train %d -> %d rows",
             len(val_df), len(new_val_df), len(train_df), len(new_train_df))
...
def main() -> None:
    ...
    ap.add_argument("--train", default="../data/bskt/current_datasets/training_dataset_smooth_tracker_z_crossover_3x.csv",
